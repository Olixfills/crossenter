...
export default function Filmstrip() {
  const { draftSlides, selectedSlideId, setSelectedSlideId, addSlide, removeSlide } = useEditorStore()
  ...
  return (
    <div className="flex flex-col h-full bg-bg-raised border-r border-border-dim w-64 shrink-0 overflow-hidden">
      <div className="p-4 border-b border-border-dim bg-bg-base/30 flex items-center justify-between">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-text-hi">Slides</h3>
        <span className="text-[10px] font-bold text-text-ghost">{draftSlides.length}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
          ...
          >
            ...
            <div className="w-full h-full p-2 flex flex-col items-center justify-center text-center">
              <p className="text-[8px] leading-tight text-white/60 line-clamp-3">
                {slide.text || <span className="italic opacity-30 whitespace-nowrap">Empty Slide</span>}
              </p>
            </div>

            {/* Slide Category Badge */}
            <div className="absolute bottom-1.5 left-1.5 z-10">
               <span className={`text-[7px] font-black px-1.5 py-0.5 rounded shadow-sm uppercase ${SLIDE_TYPE_STYLES[slide.type.toLowerCase()] || ''}`}>
                 {slide.type}
               </span>
            </div>
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-text-ghost group-hover:text-accent">New slide</span>
        </button>
      </div>
    </div>
  )
