...
export default function OverlaysView() {
  const [activeSubTab, setActiveSubTab] = useState<"blank" | "logo" | "timer" | "safety">("blank");
    ...
    <div className="flex h-full bg-bg-raised overflow-hidden">
      ...
      <aside className="w-52 border-r border-border-dim bg-bg-base/30 flex flex-col shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto py-1 custom-scrollbar">
          {[
            { id: "blank", label: "Blank Screen", icon: Square, active: state.isBlanked },
            { id: "logo", label: "Logo Overlay", icon: ImageIcon, active: state.isLogoEnabled },
            { id: "timer", label: "Quick Timer", icon: Clock, active: state.isTimerEnabled },
            { id: "safety", label: "Safety Layer", icon: ShieldCheck, active: state.isSafetyEnabled },
          ].map((tab) => (
            <button
              key={tab.id}
            ...
            >
              <div className="flex items-center gap-3">
                <tab.icon size={13} className={activeSubTab === tab.id ? "text-accent" : "text-text-ghost opacity-40"} />
        ...
        <div className="grid grid-cols-12 gap-8 h-full max-w-[1500px] overflow-hidden">
          ...
          <div className="col-span-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar-purple pr-4 pb-12 animate-in fade-in slide-in-from-left-4 duration-300">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-border-dim/30">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-accent/10 text-accent shadow-lg shadow-accent/5">
                  {activeSubTab === "blank" && <Square size={18} strokeWidth={2.5} />}
                  {activeSubTab === "logo" && <ImageIcon size={18} strokeWidth={2.5} />}
                  {activeSubTab === "timer" && <Clock size={18} strokeWidth={2.5} />}
                  {activeSubTab === "safety" && <ShieldCheck size={18} strokeWidth={2.5} />}
                </div>
                <div>
                  <h2 className="text-xs font-black text-text-hi uppercase tracking-widest capitalize">{activeSubTab} Settings</h2>
                  <p className="text-[10px] text-text-ghost font-bold opacity-50">Configure global {activeSubTab} layer</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-3 bg-bg-surface/30 rounded-2xl border border-border-dim/30">
               <button 
                  onClick={() => activeSubTab === 'blank' ? state.toggleBlank() : toggleOverlay(activeSubTab as any)}
                  className={`
                    px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] transition-all shadow-md flex-1
                    ${((activeSubTab === 'blank' ? state.isBlanked : (state as any)[`is${activeSubTab.charAt(0).toUpperCase() + activeSubTab.slice(1)}Enabled`])) 
                  ...
                  `}
                >
                  {((activeSubTab === 'blank' ? state.isBlanked : (state as any)[`is${activeSubTab.charAt(0).toUpperCase() + activeSubTab.slice(1)}Enabled`])) ? 'Active' : 'Enable Layer'}
                </button>
          ...
          <div className="col-span-4 flex flex-col gap-4 animate-in fade-in slide-in-from-right-4 duration-300 sticky top-0 h-fit">
             <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[10px] font-black text-text-ghost uppercase tracking-widest">Live Integration Preview</h3>
             </div>
             ...
             <div className="relative w-full aspect-video rounded-3xl border border-border-dim bg-black overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/5">
                ...
                {state.isSafetyEnabled && state.safetyUrl && activeSubTab !== 'blank' && (
                  <div className="absolute inset-0 z-[5]">
                    <img src={resolveMediaUrl(state.safetyUrl)} className="w-full h-full object-cover" alt="Safety" />
                  </div>
                )}

                 {/* Logo Layer (Preview) */}
                 {state.isLogoEnabled && state.logoUrl && activeSubTab !== 'blank' && (
                   <div className={`absolute z-[110] transition-all duration-500 flex flex-col pointer-events-none ${state.logoIsFullScreen ? 'inset-0 items-center justify-center p-0' : `p-4 ${getPreviewPositionStyle(state.logoPosition)}`}`}>
                     <img 
                       src={resolveMediaUrl(state.logoUrl)} 
                       style={state.logoIsFullScreen ? {
                         maxWidth: '100%',
                         maxHeight: '100%',
                         width: 'auto',
                         height: 'auto',
                ...
                {state.isTimerEnabled && activeSubTab !== 'blank' && (
                  <div className={`absolute z-[105] p-6 flex flex-col pointer-events-none ${getPreviewPositionStyle(state.timerPosition)}`}>
                    <p className="font-black tracking-tighter" style={{ color: state.timerColor, fontSize: `${state.timerFontSize / 2}px` }}>
                ...
                <div 
                  ...
                  style={{
                    ...(state.globalBlankType === 'color' && { backgroundColor: state.globalBlankValue }),
                    ...(state.globalBlankType === 'gradient' && { backgroundImage: state.globalBlankValue })
                  }}
                >
                  {state.globalBlankType === 'image' && state.isBlanked && (
                    <img src={resolveMediaUrl(state.globalBlankValue)} className="w-full h-full object-cover" alt="Blank" />
                  )}
                </div>
                ...
                {!state.isBlanked && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-10 opacity-20 pointer-events-none px-12">
                     <p className="text-white text-3xl font-black text-center leading-tight">ACTIVE OUTPUT FEED STANDBY</p>
             ...
             <div className="mt-2 p-4 bg-accent/5 border border-accent/20 rounded-2xl flex items-start gap-4 shadow-sm ring-1 ring-white/5">
                ...
                <div>
                   <p className="text-[10px] leading-relaxed text-text-lo font-medium opacity-80">
                     Live view of active layers. Adjust positioning, scale, and visuals to match your broadcast. Top toggles activate layers globally.
                   </p>
                </div>
             </div>
          </div>
      ...
      <div className="space-y-6 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button
              key={type.id}
              onClick={() => setBlankSetting('global_blank_type', type.id as any)}
              className={`
              ...
              `}
            >
              <type.icon size={14} strokeWidth={state.globalBlankType === type.id ? 3 : 2} />
          ...
          {state.globalBlankType === 'color' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex items-center gap-2">
                <input 
                  type="color" 
                  value={state.globalBlankValue.startsWith('#') ? state.globalBlankValue : '#000000'} 
                  onChange={(e) => setBlankSetting('global_blank_value', e.target.value)}
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {PRESET_GRADIENTS.map((g, i) => (
                  <button 
                    ...
                    style={{ background: g }}
                  >
                    <span className="text-[8px] font-black text-white/40 uppercase tracking-widest">Style {i+1}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {state.globalBlankType === 'image' && (
            <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
               <button 
                ...
                >
                  <Upload size={18} className="group-hover:text-accent group-hover:translate-y-[-1px] transition-all" />
                  <span className="text-[9px] font-black uppercase tracking-widest">Import Asset</span>
                </button>
                {state.globalBlankValue && state.globalBlankValue.includes('/') && (
                   <div className="flex items-center gap-2 p-2 bg-bg-base/50 rounded-lg border border-border-dim/50">
                      ...
                      <p className="text-[9px] font-bold text-text-lo truncate flex-1 uppercase tracking-tight">{state.globalBlankValue.split('/').pop()}</p>
                      <button onClick={() => setBlankSetting('global_blank_value', '#000000')} className="text-text-ghost hover:text-red-500">
                         <Trash2 size={10} />
                      </button>
                   </div>
                )}
     ...
     return (
       <div className="space-y-6">
          <section className="space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
             <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Brand Identity</h3>
             <div className="flex gap-4">
                <div className="w-32 aspect-[4/3] rounded-xl border border-border-dim bg-bg-base overflow-hidden relative group shrink-0">
                   {state.logoUrl ? (
                      <>
                        <img src={`crossenter://${state.logoUrl}`} className="w-full h-full object-contain p-2" alt="Logo" />
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                          <button onClick={() => handleImageSelect('logoUrl')} className="p-1.5 bg-accent text-white rounded-lg">
                            <Upload size={14} />
                          </button>
                        </div>
                      </>
                   ) : (
                      <button onClick={() => handleImageSelect('logoUrl')} className="w-full h-full flex flex-col items-center justify-center gap-1.5 text-text-ghost hover:bg-accent/5 transition-colors">
                         <Upload size={16} />
                         <span className="text-[8px] font-bold uppercase">PNG / SVG</span>
                      </button>
                   )}
                </div>
                <div className="flex-1 flex flex-col justify-center gap-2">
                   ...
                   <div className="flex items-center justify-between p-2 bg-bg-base/40 rounded-lg border border-border-dim/50 mb-1">
                      <span className="text-[9px] font-black text-text-ghost uppercase tracking-widest">Full Screen Mode</span>
                      <button 
                        onClick={() => setOverlaySetting('logoIsFullScreen', !state.logoIsFullScreen)}
                      ...
                      >
                         <div className={`absolute top-0.5 w-2.5 h-2.5 rounded-full bg-white shadow-sm transition-all duration-300 ${state.logoIsFullScreen ? 'translate-x-[18px]' : 'translate-x-[2px]'}`} />
                      </button>
                   </div>

                   <div className={`space-y-1.5 transition-opacity ${state.logoIsFullScreen ? 'opacity-20 pointer-events-none' : ''}`}>
                      <div className="flex justify-between items-center px-0.5">
                         <label className="text-[8px] font-bold text-text-ghost uppercase tracking-widest">Scale Factor</label>
                         <span className="text-[9px] font-mono text-accent">{state.logoScale}%</span>
                      </div>
                      <input 
                        type="range" min="10" max="300" 
                        value={state.logoScale} 
                        onChange={(e) => setOverlaySetting('logoScale', Number(e.target.value))}
                        className="w-full accent-accent h-1.5"
                      />
                   </div>
                   <div className="space-y-1.5">
                      <div className="flex justify-between items-center px-0.5">
                         <label className="text-[8px] font-bold text-text-ghost uppercase tracking-widest">Layer Alpha</label>
                         <span className="text-[9px] font-mono text-accent">{Math.round(state.logoOpacity * 100)}%</span>
                      </div>
                      <input 
                        type="range" min="0" max="1" step="0.05"
                        value={state.logoOpacity} 
                        onChange={(e) => setOverlaySetting('logoOpacity', Number(e.target.value))}
                        className="w-full accent-accent h-1.5"
                      />
                   </div>
                </div>
          ...
          <section className={`space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30 transition-opacity ${state.logoIsFullScreen ? 'opacity-20 pointer-events-none' : ''}`}>
              <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Layout Anchoring</h3>
              <div className="grid grid-cols-3 gap-1.5 w-40 bg-bg-base/40 p-1.5 rounded-xl border border-border-dim/50 shadow-inner">
                 {POSITIONS.map(pos => (
                   <button 
                     ...
                     `}
                   >
                     <div className={`w-1.5 h-1.5 rounded-full ${state.logoPosition === pos ? 'bg-white' : 'bg-current opacity-10'}`} />
                   </button>
                 ))}
              </div>
          </section>
      ...
      <div className="space-y-6">
        <section className="grid grid-cols-2 gap-4">
           <div className="space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
              <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Timer Mode</h3>
           ...
           <div className="space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
              <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Configuration</h3>
              <input 
        ...
        <section className="space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
           <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Atmosphere & Depth</h3>
           <div className="grid grid-cols-2 gap-6">
              <div className="space-y-1.5">
                 <label className="text-[8px] font-bold text-text-ghost uppercase tracking-widest">Chrome Color</label>
                 <div className="flex gap-2">
                    <input type="color" value={state.timerColor} onChange={(e) => setOverlaySetting('timerColor', e.target.value)} className="w-8 h-8 rounded-lg bg-transparent cursor-pointer p-0" />
                    <input type="text" value={state.timerColor} onChange={(e) => setOverlaySetting('timerColor', e.target.value)} className="flex-1 bg-bg-base border border-border-dim rounded-lg px-2 text-[10px] font-mono" />
                 </div>
              </div>
              <div className="space-y-1.5">
                 <div className="flex justify-between items-center px-0.5">
                    <label className="text-[8px] font-bold text-text-ghost uppercase tracking-widest">Visual Scale</label>
                    <span className="text-[9px] font-mono text-accent">{state.timerFontSize}px</span>
                 </div>
                 <input 
                   type="range" min="12" max="200" 
                   value={state.timerFontSize} 
                   onChange={(e) => setOverlaySetting('timerFontSize', Number(e.target.value))}
                   className="w-full accent-accent h-1.5"
                 />
              </div>
           </div>
        ...
        <section className="space-y-3 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
            <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Anchoring</h3>
            <div className="grid grid-cols-3 gap-1.5 w-40 bg-bg-base/40 p-1.5 rounded-xl border border-border-dim/50 shadow-inner">
                {POSITIONS.map(pos => (
                  <button 
                    key={pos}
                    ...
                    `}
                  >
                    <div className={`w-1.5 h-1.5 rounded-full ${state.timerPosition === pos ? 'bg-white' : 'bg-current opacity-10'}`} />
                  </button>
                ))}
            </div>
        </section>
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
         <section className="space-y-4 p-5 bg-bg-base/20 rounded-2xl border border-border-dim/30">
            <h3 className="text-[9px] font-black text-text-ghost uppercase tracking-widest px-1">Standby Visualization</h3>
            {state.safetyUrl ? (
              <div className="relative aspect-video rounded-2xl border border-border-dim overflow-hidden bg-black group shadow-lg ring-1 ring-white/5 mx-auto w-full">
                <img src={`crossenter://${state.safetyUrl}`} className="w-full h-full object-cover" alt="Safety standby" />
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm">
                   <button onClick={() => handleImageSelect('safetyUrl')} className="p-3 bg-accent text-white rounded-xl">
                      <Upload size={20} />
                   </button>
                </div>
              </div>
              ...
              >
                <div className="p-4 bg-bg-base rounded-full shadow-inner opacity-40 group-hover:opacity-100 transition-opacity">
                   <Upload size={24} className="group-hover:text-accent transition-colors" />
                </div>
                <span className="text-[9px] font-black uppercase tracking-widest opacity-40 group-hover:opacity-100">Upload Protective Layer Asset</span>
              </button>
            )}
         </section>
         ...
         <div className="p-5 bg-amber-500/[0.03] border border-amber-500/20 rounded-2xl flex items-start gap-4 shadow-sm">
            ...
            <p className="text-[10px] leading-relaxed text-text-lo/70 font-medium">
              The <strong>Safety Layer</strong> is your last line of defense. It renders behind overlays but in front of content. Use high-contrast standby graphics to maintain visual integrity during technical transitions.
            </p>
         </div>
      </div>
    );
